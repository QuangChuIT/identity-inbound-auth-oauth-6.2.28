...
import org.wso2.carbon.identity.oidc.session.servlet.OIDCCustomServlet;
...
public class OIDCSessionManagementComponent {
    ...
    protected void activate(ComponentContext context) {
        ...
        Servlet customServlet = new ContextPathServletAdaptor(new OIDCCustomServlet(),
                OIDCSessionConstants.OIDCEndpoints.OIDC_CUSTOM_SERVLET);

        try {
            httpService.registerServlet(OIDCSessionConstants.OIDCEndpoints.OIDC_CUSTOM_SERVLET, customServlet, null,
                    null);
        } catch (Exception e) {
            String msg = "Error when registering OIDC Logout Servlet via the HttpService.";
            log.error(msg, e);
